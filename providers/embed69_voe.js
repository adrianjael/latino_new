...
async function resolveVoe(url) {
    try {
        ...
        if (m3u8) return { url: m3u8[1], quality: "HD" };
    } catch (e) {}
    return null;
}
...
async function getStreams(tmdbId, mediaType, season, episode) {
    try {
        ...
        const embed = lat.sortedEmbeds.find(e => e.servername.toLowerCase() === "voe");
        if (!embed || !embed.link) return [];

        const payload = JSON.parse(safeAtob(embed.link.split(".")[1].replace(/-/g, "+").replace(/_/g, "/")));
        const res = await resolveVoe(payload.link);
        if (res) return [{ name: `Embed69 - VOE`, language: "Latino", quality: res.quality, url: res.url }];
    } catch (e) {}
    return [];
}
