...
async function resolveFilemoon(url) {
    try {
        ...
        const signature = typeof __crypto_ecdsa_sign_secp256r1 === "function" ? __crypto_ecdsa_sign_secp256r1(nonce) : null;
        if (signature) {
            const sig = JSON.parse(signature);
            const headers = { "Referer": "https://filemoon.sx/", "X-Embed-Parent": "https://embed69.org/" };
            const playback = await (await fetch(`https://filemoon.sx/api/playback/${id}?nonce=${nonce}&sig=${sig.signature}&x=${sig.x}&y=${sig.y}`, { headers })).json();
            if (playback.sources) return { url: playback.sources[0].url, quality: "1080p" };
        }
        ...
        if (p) {
            ...
            if (m3u8) return { url: m3u8[1], quality: "Auto" };
        }
    } catch (e) {}
    return null;
}
...
async function getStreams(tmdbId, mediaType, season, episode) {
    try {
        ...
        const embed = lat.sortedEmbeds.find(e => e.servername.toLowerCase() === "filemoon");
        if (!embed || !embed.link) return [];

        const payload = JSON.parse(safeAtob(embed.link.split(".")[1].replace(/-/g, "+").replace(/_/g, "/")));
        const res = await resolveFilemoon(payload.link);
        if (res) return [{ name: `Embed69 - Filemoon`, language: "Latino", quality: res.quality, url: res.url, headers: { "Referer": "https://filemoon.sx/" } }];
    } catch (e) {}
    return [];
}
