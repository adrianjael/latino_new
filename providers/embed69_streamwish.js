...
async function resolveStreamwish(url) {
    try {
        ...
        const m3u8 = html.match(/file:"(https?:\/\/[^"]+?\.m3u8[^"]*?)"/i);
...
async function getStreams(tmdbId, mediaType, season, episode) {
    try {
        ...
        const embed = lat.sortedEmbeds.find(e => e.servername.toLowerCase() === "streamwish");
        if (!embed || !embed.link) return [];

        const payload = JSON.parse(safeAtob(embed.link.split(".")[1].replace(/-/g, "+").replace(/_/g, "/")));
        const res = await resolveStreamwish(payload.link);
        if (res) return [{ name: `Embed69 - Streamwish`, language: "Latino", quality: res.quality, url: res.url }];
    } catch (e) {}
    return [];
}
